  ...
  module Extend
    module String
      ...
      def convert_utf8_to_sjis
        ...
        self.encode(Encoding::SJIS.to_s, Encoding::UTF_8.to_s, option)
      end
      ...
      def convert_sjis_to_utf8
        # SJIS に存在しない文字は、スペースに変換する
        option = {invalid: :replace, undef: :replace, replace: ' '}
        self.encode(Encoding::UTF_8.to_s, Encoding::SJIS.to_s, option)
      end

      # @return [String] 文字列(UTF8)
      def convert_utf16le_to_utf8
        option = {invalid: :replace, undef: :replace, replace: ' '}
        self.encode(Encoding::UTF_8.to_s, Encoding::UTF_16LE.to_s, option)
      end
